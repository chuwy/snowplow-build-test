...
object CrossBatchDeduplicationSpec {
  ...
  object Storage {
    ...
    private def getStorage() = {
      val config = getDynamodbConfig.map { case (accessKeyId, secretAccessKey, awsRegion, dynamodbTable) =>
        try {   // Send request to delete previously created table and wait unit it is deleted
          ...
          client.deleteTable(dynamodbTable)
          new Table(client, dynamodbTable).waitForDelete()
        } catch {
